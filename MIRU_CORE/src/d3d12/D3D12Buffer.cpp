	...
	:m_Device(reinterpret_cast<ID3D12Device*>(pCreateInfo->device))
{
	...
	{
		m_Allocation.width = Align<size_t>(m_CI.size, D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT);
	}
	else if (m_CI.imageDimension.width % D3D12_TEXTURE_DATA_PITCH_ALIGNMENT)
	{
		m_Allocation.rowPitch = Align<size_t>((m_CI.imageDimension.width * m_CI.imageDimension.pixelSize), D3D12_TEXTURE_DATA_PITCH_ALIGNMENT);
	...
	:m_Device(reinterpret_cast<ID3D12Device*>(pCreateInfo->device))
{
	...
	switch (m_CI.type)
	{
		...
		case Type::UNIFORM:
		{
			...
			m_CBVDesc.SizeInBytes = Align<UINT>(static_cast<UINT>(m_CI.size), D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT);
		}
