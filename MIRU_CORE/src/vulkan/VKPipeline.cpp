	...
	:m_Device(*reinterpret_cast<VkDevice*>(pCreateInfo->device))
{
	...
	for(auto& descriptorSetLayout : m_CI.layout.descriptorSetLayouts)
		vkDescriptorSetLayouts.push_back(std::dynamic_pointer_cast<DescriptorSetLayout>(descriptorSetLayout)->m_DescriptorSetLayout);
	...
	if (m_CI.type == crossplatform::PipelineType::GRAPHICS)
	{
		...
		for (auto& shader : m_CI.shaders)
			vkShaderStages.push_back(std::dynamic_pointer_cast<Shader>(shader)->m_ShaderStageCI);
		...
		m_GPCI.renderPass = std::dynamic_pointer_cast<RenderPass>(m_CI.renderPass)->m_RenderPass;
	...
	else if (m_CI.type == crossplatform::PipelineType::COMPUTE)
	{
		...
		m_CPCI.flags = 0;
		m_CPCI.stage = std::dynamic_pointer_cast<Shader>(m_CI.shaders[0])->m_ShaderStageCI;
