	...
	:m_Device(reinterpret_cast<ID3D12Device*>(ref_cast<DescriptorPool>(pCreateInfo->pDescriptorPool)->GetCreateInfo().device))
{
	...
	m_DescHeapCPUHandles.resize(ref_cast<DescriptorPool>(m_CI.pDescriptorPool)->m_DescriptorPools.size());
	for (size_t i = 0; i < ref_cast<DescriptorPool>(m_CI.pDescriptorPool)->m_DescriptorPools.size(); i++)
	{
		for (size_t j = 0; j < 4; j++)
		{
			...
			if (heap)
				m_DescHeapCPUHandles[i][j] = heap->GetCPUDescriptorHandleForHeapStart();
		}
	}
	...
	for (auto& descriptorSetLayout : m_CI.pDescriptorSetLayouts)
	{
		...
		for (auto& descriptorSetLayoutBinding : descriptorSetLayout->GetCreateInfo().descriptorSetLayoutBinding)
		{
			...
			{ 
				countSampler++;
				if (baseBindingSampler == -1) descriptorSetLayoutBinding.binding;
				continue; 
			}
...
void DescriptorSet::AddBuffer(uint32_t index, uint32_t bindingIndex, const std::vector<DescriptorBufferInfo>& descriptorBufferInfos, uint32_t desriptorArrayIndex)
{
	...
	for (auto& descriptorBufferInfo : descriptorBufferInfos)
	{
		...
		{
			descriptorWriteLocation = m_DescHeapCPUHandles[index][0];
			descriptorWriteLocation.ptr += (cbv_srv_uav_DescriptorSize * (bindingIndex + desriptorArrayIndex));
...
void DescriptorSet::AddImage(uint32_t index, uint32_t bindingIndex, const std::vector<DescriptorImageInfo>& descriptorImageInfos, uint32_t desriptorArrayIndex)
{
	...
	for (auto& descriptorImageInfo : descriptorImageInfos)
	{
		...
		{
			descriptorWriteLocation = m_DescHeapCPUHandles[index][2];
			descriptorWriteLocation.ptr += (rtvDescriptorSize * (bindingIndex + desriptorArrayIndex));
		...
		{
			descriptorWriteLocation = m_DescHeapCPUHandles[index][3];
			descriptorWriteLocation.ptr += (dsvDescriptorSize * (bindingIndex + desriptorArrayIndex));
		...
		{
			descriptorWriteLocation = m_DescHeapCPUHandles[index][1];
			descriptorWriteLocation.ptr += (samplerDescriptorSize * (bindingIndex + desriptorArrayIndex));
