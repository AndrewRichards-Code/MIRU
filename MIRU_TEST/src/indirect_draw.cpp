...
void IndirectDraw(uint32_t maxFrames)
{
	...
	wc.lpszClassName = "MIRU_TEST_IndirectDraw";
	RegisterClass(&wc);
	...
	indirectArguments.drawIndexedIndirectCommand.indexCount = 36;
	indirectArguments.drawIndexedIndirectCommand.instanceCount = 1;
	indirectArguments.drawIndexedIndirectCommand.firstIndex = 0;
	indirectArguments.drawIndexedIndirectCommand.vertexOffset = 0;
	indirectArguments.drawIndexedIndirectCommand.firstInstance = 0;
	indirectArguments.drawIndirectCommand.vertexCount = 3;
	indirectArguments.drawIndirectCommand.instanceCount = 1;
	indirectArguments.drawIndirectCommand.firstVertex = 0;
	indirectArguments.drawIndirectCommand.firstInstance = 0;
	Buffer::CreateInfo indirectArgBufferCI;
	indirectArgBufferCI.debugName = "Indirect Argument Buffer";
	indirectArgBufferCI.device = device;
	indirectArgBufferCI.usage = Buffer::UsageBit::INDIRECT_BIT;
	indirectArgBufferCI.size = sizeof(IndirectArguments);
	indirectArgBufferCI.data = &indirectArguments;
	indirectArgBufferCI.allocator = cpu_alloc_0;
	BufferRef indirectArgBuffer = Buffer::Create(&indirectArgBufferCI);
	...
	resolveAndInputImageCI.layout = GraphicsAPI::IsD3D12() ? Image::Layout::SHADER_READ_ONLY_OPTIMAL : Image::Layout::UNKNOWN;
	...
	p1CI.dynamicRendering = { 0, { swapchain->m_SwapchainImages[0]->GetCreateInfo().format}, Image::Format::UNKNOWN, Image::Format::UNKNOWN };
	...
	while (!g_WindowQuit && frameCount < maxFrames)
	{
...

		{
			...
			cmdBuffer->DrawIndexedIndirect(frameIndex, indirectArgBuffer, offsetof(IndirectArguments, drawIndexedIndirectCommand), 1, sizeof(DrawIndexedIndirectCommand));
			...
			cmdBuffer->DrawIndirect(frameIndex, indirectArgBuffer, offsetof(IndirectArguments, drawIndirectCommand), 1, sizeof(DrawIndirectCommand));
			cmdBuffer->EndRendering(frameIndex);
